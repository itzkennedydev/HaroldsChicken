...
// Static value props data moved outside component
const VALUE_PROPS_DATA: ValuePropItem[] = [
    {
      id: 1,
...
// Memoized value prop item component
const ValuePropCard = memo(function ValuePropCard({
  ...
  const containerClasses = useMemo(() =>
    `flex flex-col-reverse md:flex-row ${prop.imagePosition === 'left' ? 'md:flex-row-reverse' : ''} items-center gap-8 md:gap-16 transition-all duration-700 ${
      inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
    }`,
    [prop.imagePosition, inView]
  );

  const imageContainerClasses = useMemo(() =>
    `relative w-full aspect-[4/3] md:h-[400px] rounded-lg overflow-hidden transition-transform duration-500 hover:scale-[1.02] ${
...
function ValuePropComponent() {
  ...
  // Memoized handlers for each card
  const createHandlers = useCallback((id: number) => ({
    ...
    >
      <Container>
        <div className="space-y-16 md:space-y-24">
          {VALUE_PROPS_DATA.map((prop, index) => {
            const handlers = createHandlers(prop.id);
            return (
              <div key={prop.id} ref={refs[index]}>
                <ValuePropCard
                  prop={prop}
...
export const ValueProp = memo(ValuePropComponent);